...
# Load the model
model_path = "./model/Phi-3.5-mini-instruct-Q4_0_4_4.gguf"
    ...
    with open(output_file, 'w', newline='', encoding='utf-8') as csvfile:
        ...
        for message_index, message in enumerate(messages):
            if isinstance(message, str):
                print(f"Processing message {message_index}: {message[:50]}...")  # Print first 50 chars
                #test_output = llm(message)
                #print("Test response:", test_output)
                start_time = time.time()
                try:
                    llama_output, output, stderr_output = capture_output(lambda: llm(message, max_tokens=500))
                except Exception as e:
                    print(f"Error processing message {message_index}: {e}")
                    continue
                end_time = time.time()
